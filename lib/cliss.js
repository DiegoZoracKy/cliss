...
function buildOptionsFromParametersAndSpec(action, optionsSpec) {
	const inspectedParameters = inspectParameters(action);	
	const parametersNames = getParametersNames(action) || [];	
	
	const inspectedParametersFlattenedDestructuring = [];
	if(inspectedParameters){
		inspectedParameters.forEach(inspectedParameter => {
			if(inspectedParameter.expectsDestructuring){
				inspectedParameter.destructuredParameters.forEach(destructuredParameter => {
					inspectedParametersFlattenedDestructuring.push(destructuredParameter);
				});
			} else {
				inspectedParametersFlattenedDestructuring.push(inspectedParameter);
			}
		});
	}
	
	const optionsFromParameters = parametersNames.map((parameter, i) => {
		const option = { name: parameter };

		if (inspectedParametersFlattenedDestructuring[i].isRestParameter) {
